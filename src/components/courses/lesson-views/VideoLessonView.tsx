...
export default function VideoLessonView({
  ...
  // Enhanced retry function with progressive backoff
  const retryVideoLoad = useCallback(() => {
    if (videoRef.current && video?.source) {
      ...
      const currentSrc = video.source;
      setLoadingSourceUrl(currentSrc);
      
      // Clear the video source
      videoRef.current.src = '';
      videoRef.current.load();
      
      console.log(`[VIDEO_RETRY] Manual retry attempt ${loadRetries + 1} of ${MAX_RETRIES}`);
      
      // Add timestamp to URL to bypass cache
      const cacheBuster = `${currentSrc.includes('?') ? '&' : '?'}_t=${Date.now()}`;
      const urlWithCacheBuster = currentSrc + cacheBuster;
      
      // Short timeout to ensure DOM updates with exponential backoff
      const backoffDelay = Math.min(200 * Math.pow(1.5, loadRetries), 2000);
      ...
      setTimeout(() => {
        if (videoRef.current) {
          ...
          videoRef.current.play().catch(e => {
            console.warn('[VIDEO_RETRY] Auto-play failed after retry:', e);
          });
        }
      }, backoffDelay);
    }
  }, [video?.source, loadRetries]);
  
  // Auto-retry on initial load failures
  useEffect(() => {
    let retryTimeout: NodeJS.Timeout;
    
    if (videoLoadFailed && autoRetryAttempts < 3 && loadRetries < MAX_RETRIES) {
      console.log(`[VIDEO_AUTO_RETRY] Scheduling auto-retry ${autoRetryAttempts + 1} in ${AUTO_RETRY_DELAY}ms`);
  ...
  }, [videoLoadFailed, autoRetryAttempts, loadRetries, retryVideoLoad]);
  
  // Reset retry counters when video source changes
  useEffect(() => {
    if (video?.source && video.source !== loadingSourceUrl) {
      setLoadRetries(0);
      setAutoRetryAttempts(0);
      setVideoLoadFailed(false);
      setLoadingSourceUrl(video.source);
    }
  }, [video?.source, loadingSourceUrl]);
  
  // Enhanced fetch logic with better error handling
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cleanVideoId = videoId ? videoId.replace(/[^a-zA-Z0-9-]/g, '') : videoId;
        
        // Log if videoId was cleaned/modified
        if (videoId && cleanVideoId !== videoId) {
          console.log('[VIDEO_LESSON_VIEW] Cleaned videoId for API request:', 
            { original: videoId, cleaned: cleanVideoId });
        }
        ...
        try {
          ...
          // Continue with video fetch even if lesson data fails
        }
        ...
        try {
          console.log(`[VIDEO_FETCH] Fetching video data for ID: ${cleanVideoId}`);
          
          // Fetch video data
          const videoResponse = await axios.get(`/api/videos/${cleanVideoId}`, {
          ...
          if (videoResponse.data.status === 200) {
            ...
            console.log(`[VIDEO_FETCH] Successfully fetched video data:`, {
              title: videoData.title,
              hasSource: Boolean(videoData.source),
              sourceUrl: videoData.source?.substring(0, 50) + '...',
              processing: videoData.processing,
            });
            
            setVideo(videoData);
            setLoadingSourceUrl(videoData.source);
            ...
            const errorMsg = `Failed to load video: ${videoResponse.data.error || videoResponse.data.message || 'Unknown error'}`;
            console.error(errorMsg, { responseData: videoResponse.data });
            setError(errorMsg);
          ...
          console.error("Video fetch error details:", errorDetails);
  ...
  useEffect(() => {
    ...
    // Enhanced video error handler defined within the useEffect
    const handleError = (e: ErrorEvent) => {
      ...
      if (videoRef.current) {
        console.log('[VIDEO_ERROR] Video element state:', {
          ...
          currentSrc: videoRef.current.currentSrc.substring(0, 50) + '...',
    ...
    const handleSuspend = () => console.log('[VIDEO_EVENT] Suspend: Media data loading has been suspended');
    const handleEmptied = () => console.log('[VIDEO_EVENT] Emptied: Media element suddenly became empty');
    const handleAbort = () => console.log('[VIDEO_EVENT] Abort: Media data download aborted');
  ...
  }, [savedProgress, onProgress, onComplete, isCompleted]);
  ...
  const navigateToCourse = () => {
    if (courseId) {
      ...
      // Extract from URL params if not available from props
      router.push(`/courses/${params.courseId}`);
    } else {
      // Fallback to courses list if we can't determine the course
      router.push('/courses');
    }
  };
  ...
  const navigateToAiTutor = () => {
    if (videoId) {
      // Clean the videoId to prevent malformed URLs
      const cleanedVideoId = videoId.replace(/[^a-zA-Z0-9-]/g, '');
      
      // Log original and cleaned ID for debugging
      if (videoId !== cleanedVideoId) {
        console.log('[AI_TUTOR_NAVIGATION] Cleaned videoId for navigation:', 
          { original: videoId, cleaned: cleanedVideoId });
      }
      
      router.push(`/ai-tutor?videoId=${cleanedVideoId}`);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <LessonNavigation 
        courseId={courseId || (params?.courseId ? params.courseId.toString() : undefined)}
        courseTitle={courseTitle || (courseStructure?.title ?? "Course")}
        sectionTitle={sectionTitle}
        lessonTitle={lessonTitle || video?.title || "Lesson"}
        ...
        <div className="md:col-span-2">
          <div className="bg-card rounded-lg border border-border overflow-hidden elevation-3 elevation-transition">
            <Tabs value={videoTabs} onValueChange={setVideoTabs} className="w-full">
                ...
                <div className="relative bg-black">
                  ...
                  {(video?.title || lessonTitle) && (
                    <div className="absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-background/80 to-transparent p-4">
                      <h2 className="text-xl font-medium text-white">{video?.title || lessonTitle}</h2>
                    </div>
                  )}
                  ...
                  <video
                    ...
                    playsInline
                    preload="metadata" // Start with metadata only for faster initial load
                    crossOrigin="anonymous" // Help with CORS issues
                    onError={() => setVideoLoadFailed(true)}
                    onCanPlay={() => {
                      // When video can play, update loading state and reset error flags
                      setLoading(false);
                      setVideoLoadFailed(false);
                      console.log('[VIDEO_EVENT] CanPlay: Video can begin playback');
                      
                      // Force preload to auto once we know it can play
                      if (videoRef.current) {
                        videoRef.current.preload = "auto";
                      }
                    }}
                    onStalled={() => {
                      console.log("[VIDEO_EVENT] Stalled: Video playback stalled");
                    ...
                    onWaiting={() => {
                      console.log("[VIDEO_EVENT] Waiting: Video waiting for data");
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="relative flex items-center w-24">
                            <input
                              ...
                              className="w-full h-1.5 bg-muted/50 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-primary"
                            />
                          </div>
                        </div>
              ...
              {/* Add prominent Back to Course button */}
              ...
              <TabsContent value="transcript" className="m-0 p-4 max-h-[500px] overflow-y-auto custom-scrollbar">
                {/* Transcript content */}
